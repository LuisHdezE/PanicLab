...
data class PanicCode(
    ...
    companion object {
        fun parse(raw: String): PanicCode? {
            ...
            val numeric = HexUtils.parseCodeToLong(trimmed) ?: return null
            return PanicCode(
                rawValue = trimmed,
                numericValue = numeric,
                hexadecimal = HexUtils.toCanonicalHex(trimmed),
                decimal = numeric.toString(10)
            )
        }

        fun fromNumeric(value: Long): PanicCode = PanicCode(
            rawValue = value.toString(10),
            numericValue = value,
            hexadecimal = HexUtils.toCanonicalHex("0x" + value.toString(16)),
...
enum class RulePackOrigin { PANICLAB_OFFICIAL, USER_IMPORTED, BUNDLED }
...
data class ValidationIssue(
    ...
    enum class IssueType { ERROR, WARNING }
}
...
data class RuleDiffItem(
    ...
    enum class ChangeType { ADDED, MODIFIED, REMOVED, DEACTIVATED, UNCHANGED }
}
